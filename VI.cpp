...
void VI_UpdateSize()
{
	...
	VI.width = (uint32_t)((hEnd - hStart) * xScale);
	VI.height = (uint32_t)((vEnd - vStart) * yScale * 1.0126582f);

	if (VI.width == 0.0f) VI.width = (uint32_t)320.0f;
	if (VI.height == 0.0f) VI.height = (uint32_t)240.0f;
}
